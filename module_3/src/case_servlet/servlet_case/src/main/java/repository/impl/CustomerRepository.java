...
public class CustomerRepository implements ICustomerRepository {
    ...
    public static final String SELECT_ALL_CUSTOMER_TYPE = "select * from customer_type where is_delete=0;";
    ...
    @Override
    public List<CustomerType>
    selectAllCustomerType() {
        ...
        try {
            ...
            ResultSet rs = preparedStatement.executeQuery();
            while (rs.next()) {
                int customerTypeId = rs.getInt("id");
                String name = rs.getString("name");

                customerTypeList.add(new CustomerType(customerTypeId, name));
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
